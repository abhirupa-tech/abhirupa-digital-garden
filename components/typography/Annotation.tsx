...
export function Annotation({ trigger, note, triggerClassName }: AnnotationProps) {
  ...
  return (
    <span className="relative inline-block">
      ...
      <span
        role="tooltip"
        className={`pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max max-w-xs origin-bottom -translate-x-1/2 rounded-xs border border-parchment/15 bg-ink-800/95 px-3 py-2 text-left font-body text-sm leading-snug text-parchment-muted shadow-lg backdrop-blur-xs transition-all duration-200 ease-out ${
