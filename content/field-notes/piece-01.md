...
a) Communication — How do agents reliably talk to each other?

b) Orchestration — Do we even need a central orchestrator, or can agents self-organize?

c) Adaptability — How do we make the whole system responsive to the current state of the world, not the state we designed for last week?
...
Amdahl's law says something simple and slightly annoying — the speedup you get from parallelizing a system is capped by the part of it that can't be parallelized. If 30% of your workflow is sequential, even infinite parallel workers only get you about a 3.3x speedup. The serial fraction is the ceiling.

P = the parallelizable fraction. N = number of workers. As N → ∞, the ceiling is 1/(1-P).
...
A DAG is a directed acyclic graph where each node represents a unit of work and each edge represents a dependency — and no path through the graph ever loops back on itself. A fan-out edge is one node branching out to many — one source, many targets, parallel work dispatched. A fan-in edge is many nodes converging into one — many sources, one target, parallel work synchronized.
...
That's the whole trick. Stan doesn't need a hardcoded sequence. Stan needs a graph and a topological sort. The DAG describes what depends on what; Kahn's decides when.

And here's where Amdahl's law closes the loop. The fridge, pantry, and preferences agents running in parallel is exactly the optimization Amdahl's promised — a chunk of the workflow that doesn't have to be sequential, suddenly isn't. The serial parts (the recipe agent waiting on its three parents, the shopping agent waiting on the recipe) are the unavoidable floor. Everything above that floor is yours to compress.
